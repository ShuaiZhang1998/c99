...
- 基本类型：`int` / `char` / `short` / `long` / `long long` / `_Bool` / `long double`
- 无符号：`unsigned char/short/int/long/long long`
- 浮点：`float` / `double`
- 指针（多级）：`int*`, `int**`, `void*`
- 数组（含多维）
- `struct` / `union`（定义、成员访问、按值传参/返回/赋值、比较，含位域）
- `typedef` 与 `enum`
...
- （暂无）目前已覆盖此前用例中的 `u/l/ll` 整数后缀。

## 面向简单 C99 项目的待实现清单（详细版）

下面按“最小可用优先级”列出仍缺的关键能力。可作为后续实现路线图：
...
- **类型系统**
  - （暂无）
- **存储类与链接**
  - `extern`（跨 TU 的声明/定义一致性与链接行为完善）
  - `static` 的更多语义边界（例如对内部链接的诊断细化）
