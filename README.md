...
- 多个函数与原型声明
- 全局变量定义
- 极简预处理器：`#include`（`"..."` 与 `<...>`，支持 `-I`/`-isystem` 搜索路径）、`#define`（含可变参数、`#`/`##`）、`#undef`、`#ifdef/#ifndef/#if/#elif/#else/#endif`
...
- 预处理器能力有限：支持 `#include "..."` 与 `<...>`（搜索顺序：当前文件目录、`-I`、`-isystem`）、对象宏与函数宏（含可变参数、`#`/`##`）、`#ifdef/#ifndef/#if/#elif/#else/#endif`；`#if` 支持整数/宏数值/`defined` 与 `!`、`+`、`-`、`*`、`/`、`%`、比较、`&&`、`||`，不支持位运算或其他复杂表达式
...
## **如果目标是编译较简单的旧 C99 项目，最重要的缺口**

- 字符/字符串字面量与字符串相关初始化（许多旧代码依赖 `"..."`、`'x'` 与字符串数组初始化）
- 多翻译单元与链接模型（`*.c` 多文件项目无法直接编译）
- `typedef`/`enum`/显式类型转换/`sizeof`（常见类型别名与常量枚举、类型计算）
- 预处理器内置宏与 include guard 更完整（`__FILE__`/`__LINE__`/`__DATE__` 等）
- 运算符缺口：自增自减、位运算/移位、复合赋值

## **尚缺的大块能力（后续扩展方向）**

- 预处理器：内置宏、include guard 语义更完整、宏展开与 token 规则更严格
- 多翻译单元与链接模型
- 类型与转换：完整整数提升/常规算术转换、显式转换、`sizeof`
- 更多类型：`enum`、`typedef`、`union`、位域、`_Bool`、`long double`
- 字面量：整数 U/L/LL 后缀、十六/八进制、字符串与字符字面量
- 初始化：数组对象初始化列表、指派初始化、字符串初始化数组
- 运算符：自增自减、位运算/移位、复合赋值
